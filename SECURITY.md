...
`ansys-cfx-mcp` exposes tools that drive a live ANSYS CFX session.
...
- **Do NOT expose the MCP server (stdio/HTTP) to an untrusted network.**
  Anyone who can reach it can drive CFX and run code in the server
  process.
...
- **Public LLM providers (OpenAI, Azure, Anthropic, Gemini, …):** nothing to
  ...
  built-in trust store (`certifi`) already recognizes, so calls just work.
