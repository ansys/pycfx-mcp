...
* An optional higher-level agent layer (when installed): Logs every
  tool invocation (name, sanitized arguments, latency, and brief result). This
...
:mod:`ansys.cfx.mcp.common.session_logging` without callers needing to
...
# Loggers callers should use. Each child of ``ansys.cfx.mcp``
# propagates up to the package logger and the session FileHandler picks
# it up.
TOOL_LOGGER = logging.getLogger("ansys.cfx.mcp.agent.tool_calls")
SESSION_LOGGER = logging.getLogger("ansys.cfx.mcp.session.run_code")
...
def summarise_result(result: Any, *, limit: int = _DEFAULT_VALUE_LIMIT) -> Any:
    ...
    nested ``state``, ``snapshot``, or ``preview`` blobs).
    Top-level keys are kept, but values are truncated aggressively so the log line
    stays under a few kilobytes. Non-dictionary returns are truncated.
