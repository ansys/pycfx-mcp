...
# ``ansys.cfx.mcp.common.backend`` is intentionally not on the path
...
#: is the leaf's active backend (a :class:`ansys.cfx.mcp.common.backend.Backend`);
#: every subsequent argument must be keyword-only with an explicit
#: type annotation so FastMCP can synthesise the input JSON schema
#: from the function signature. Returning ``dict[str, Any]`` is
#: conventional but the framework accepts any JSON-serialisable
#: payload (list, scalar, …).
...
class DomainToolSpec:
    ...
    JSON schema is only useful when the handler accepts a union /
    variant payload that can't be expressed cleanly with Python type
    hints.
